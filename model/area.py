...
class Area:
    ...
    def read(self, mute=False):
        ...
        try:
            data = ""
            with open(self.areafile, "r", encoding="utf-8-sig", errors="replace") as pf:
                data = json.load(pf)

            for area in data["list_of_area"]:
                self.list_of_area.append({"name": area.get("name", "")})
    ...
    def save(self, mute=False):
        ...
        try:
            with open(self.areafile, "w", encoding="utf-8") as pf:
    ...
    def delete_area(self, _id):
        ...
        if _id < 0 or _id >= len(self.list_of_area):
            ...
            return "Deve haver pelo menos uma área na propriedade."

        else:
            for planting in self.planting.list_of_planting:
                if planting["area"] == self.list_of_area[_id]["name"]:
                    return "Esta área está em uso por um plantio. Edite ou remova o plantio primeiro."
